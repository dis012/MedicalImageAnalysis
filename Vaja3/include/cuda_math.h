...
#ifndef SQDEPS
#   define SQDEPS 1.490116119384766e-08
#endif
...
////////////////////////////////////////////////////////////////////////////////
// Additional overloads for builtin-type 
static __inline__ __host__ __device__ uint2 make_uint2(uint* p)
{
  uint2 t; t.x = p[0]; t.y = p[1]; return t;
}       

static __inline__ __host__ __device__ float2 make_float2(float* p)
{
  float2 t; t.x = p[0]; t.y = p[1]; return t;
}       
    
static __inline__ __host__ __device__ uint3 make_uint3(uint* p)
{
  uint3 t; t.x = p[0]; t.y = p[1]; t.z = p[2]; return t;
}       

static __inline__ __host__ __device__ float3 make_float3(float* p)
{
  float3 t; t.x = p[0]; t.y = p[1]; t.z = p[2]; return t;
}   

// static __inline__ __host__ __device__ float3 cross(float3& v1, float3& v2)
// {
//   float3 t; 
...
// static __inline__ __host__ __device__ float3 cross(float2& v1, float2& v2)
// {
//   float3 t; 
//   t.x = 0.0f; t.y = 0.0f;
//   t.z = v1.x*v2.y - v1.y*v2.x;
